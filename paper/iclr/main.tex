...
The same intervention legitimately produces different accuracies under different protocols, and this paper reports several. For the 9-row repair on entity counting: \textbf{60.7\%} is the unified multi-seed protocol of Table~\ref{tab:unified_evaluation} (200 prompts $\times$ 3 seeds, digit-restricted next-token); \textbf{93.8\%} is the single-seed train/held-out split used for the cross-model panel (Table~\ref{tab:intervention_comparison}); \textbf{99.9\%} is instruct-mode first-token accuracy. Baseline values differ analogously (10.3--17.0\%) because of seed, template, and mode differences, and \textbf{38.8\%}/\textbf{38.6\%} are stratified vs.\ unified sampling of the unrepaired model (Figure~\ref{fig:probe_gap}). Every number is reproducible from its stated protocol; when a single headline figure is quoted in prose, it is always the unified multi-seed value.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.66\linewidth]{fig3_probe_r2_gap.pdf}
\caption{Representation--output gap: per-layer probe $R^2$ (blue) remains above
  ...
  the unified-evaluation sampling of Table~\ref{tab:unified_evaluation}). The shaded
...
\begin{figure}[t]
\centering
\includegraphics[width=0.88\linewidth]{logit_lens_depth.pdf}
...
A measurement caveat dominates any comparison between CoT and our interventions:
CoT accuracy is highly sensitive to how the emitted reasoning is scored. Scoring
the \emph{first} integer anywhere in the output credits intermediate counting
tokens rather than the model's actual answer. We measured both rules head-to-head
on Qwen3-8B entity counting ($N{=}200$, greedy): under first-integer scoring,
direct / zero-shot CoT / few-shot CoT score 7.5\% / 12.5\% / 21.5\% --- the
familiar impression that CoT helps; under final-integer scoring --- the rule all
generation-mode numbers in this paper use --- the same runs score
\textbf{18.5\% / 11.5\% / 11.5\%}. The apparent CoT benefit is an artifact of
crediting intermediate count tokens; under honest scoring, step-by-step prompting
with these templates \emph{reduces} accuracy relative to direct answering. Mechanistically, the corrected measurements are consistent with our diagnosis:
externalizing aggregation into emitted text does not repair the readout
bottleneck --- the model still fails to convert its count representation into
the right output token at the final position, and the extra decoding steps add
format-failure opportunities. LoRA Q/V corrects the routing \emph{internally}, enabling
...
This clarifies the paper's contribution relative to the CoT literature:
apparent CoT gains on small-count benchmarks should be audited against
final-answer scoring before they are attributed to externalized computation.
LoRA Q/V remains the only intervention we evaluate that raises honest
autoregressive accuracy (83.1\%). A full mechanistic account of CoT ---
e.g., whether count-to-\texttt{lm\_head} alignment improves across CoT
reasoning tokens, and whether filler tokens without explicit counting produce
the same effect --- is an open question beyond the scope of this paper.
...
\section{Diversity-Controlled Analysis of the Entity-Counting Repair Ceiling}
\label{sec:diversity}

Limitation~4 leaves open why the 9-row repair caps near 60\% on entity counting while other tasks reach $\geq$99\%. Two hypotheses compete: (H1)~digit-row norm competition against the full vocabulary, and (H2)\ intra-class hidden-state diversity --- the same count is encoded differently across entity words, sentence templates, and distractor loads. The two suggest different fixes (row-norm rescaling vs.\ representation normalization), so we discriminate them directly: holding the count distribution fixed (uniform 1--9), we vary only surface diversity across four controlled levels --- entity-pool size \{1, 3, 8, 20\}, template variety \{1--4\}, and distractor load \{0--4\} --- and rerun the full pipeline (extraction, ridge probe, margin-trained 9-row repair) under one local evaluator on Qwen3-8B.

\begin{table}[h]
\centering\small
\begin{tabular}{lrrrr}
\toprule
& D0 minimal & D1 low & D2 medium & D3 original \\
\midrule
Baseline (digit-restricted) & 6.3\% & 8.1\% & 5.6\% & 9.4\% \\
9-row repair (in-domain) & \textbf{70.6\%} & 51.9\% & 45.0\% & 53.8\% \\
9-row repair trained on D0 & 70.6\% & 38.8\% & 33.8\% & 42.5\% \\
\bottomrule
\end{tabular}
\caption{Diversity sweep ($N{=}240$ train / $N{=}160$ test per level, held-out prompts, digit-restricted argmax). Moving from a single-entity single-template distribution to the original diverse distribution costs the in-domain repair 16.8 percentage points; a D0-trained repair loses roughly half its accuracy out of domain.}
\label{tab:diversity}
\end{table}

The gradient supports H2 as a major contributor to the ceiling: restricting to one entity and one template raises repair accuracy from 53.8\% to 70.6\%, and a repair trained on minimal-diversity prompts transfers poorly as diversity grows (70.6\% $\to$ 33.8--42.5\%). However, the decline is not monotonic --- accuracy partially recovers from D2 to D3 --- and even at minimal diversity the ceiling persists at ${\sim}71\%$, so neither hypothesis alone explains the gap: intra-class diversity accounts for roughly a third to a half of it, with the remainder consistent with residual norm competition and capacity limits. Caveats: this analysis uses a local evaluator whose absolute baselines differ from the TPU harness (digit-restricted baselines 5.6--9.4\% here vs.\ 13.7\% unified), is single-seed, and probes were not re-fit at reduced dimensionality; we release the generator, extraction, and repair scripts for replication.
...
AI tools were used for editing assistance (grammar and wording), implementing standard data-processing utilities, and preparing visualizations. All research design, experiments, analysis, and claims are the authors' own; the authors take full responsibility for the work.
