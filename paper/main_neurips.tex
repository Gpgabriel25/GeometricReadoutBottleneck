...
from 55,980 to 1 ($50{,}000\times$ improvement). The bottleneck generalizes
...
This paper makes three nested claims:
\begin{enumerate}
\item \textbf{Geometric} (protocol-independent): Count-encoding directions are
  orthogonal to \texttt{lm\_head} digit rows ($|\cos| \leq 0.032$,
  indistinguishable from random). This is a structural property of the trained model.
\item \textbf{Causal-diagnostic} (constrained next-token): The 9-row repair
...
\item \textbf{Deployable} (autoregressive generation): LoRA Q/V rank-16 corrects
...
task requiring no external knowledge. Prior work has documented these
failures~\citep{razeghi2022impact,stolfo2023mechanistic} without providing a
principled explanation of \emph{why} counting breaks down when the answer is
directly available in the input.
...
protocol on Qwen3-8B, probe-round reaches 96.8--100.0\% and 9-row repair
...
natural-language counting (probe-round 96.3\%). MMLU and GSM8K confirm scope
specificity: the bottleneck fires when the model pre-encodes the answer
directly, not for multi-step reasoning. At 14B scale the misalignment
...
hypothesis~\citep{park2023linear} and related work on latent
knowledge~\citep{burns2023discovering} formalize this gap. Complementing
...
$6 \text{ counts} \times 3 \text{ distractors} \times 4 \text{ lengths} \times
3 \text{ spacings} = 216 \text{ conditions} \times 20 \text{ samples} = 4{,}320
\text{ prompts}$. Counts: $\{1, 2, 3, 5, 8, 12\}$; split 70/30 stratified.
For DPS evaluation we use single-digit counts $\{1, \ldots, 9\}$ (800 train,
300 test). A natural-language extension tests 8 entity categories across 8
diverse templates; 5 entity types are seen during probe training, 3 held out.
...
\emph{generation} (greedy decoding, first integer extracted), and
\emph{instruct} (chat template wrapping). Each result is labeled with its mode.
Headline comparisons use a shared protocol; cross-mode comparisons are noted
explicitly. All tables specify evaluation protocol.
...
\centering
\small
\caption{\textbf{Unified evaluation.} All methods under three standard
  protocols on Qwen3-8B entity counting.}
...
\begin{table}[t]
\centering
\caption{Per-layer probe $R^2$ on Qwen3-8B (ridge regression, entity-mean).}
\label{tab:probe_r2}
\begin{tabular}{lcc}
\toprule
...
\caption{Representation--output gap: per-layer probe $R^2$ (blue) remains above
  ...
  only 38.8\%. The shaded region is between what the model \emph{knows} and
  what it \emph{says}.}
\label{fig:probe_gap}
\end{figure}
...
($0.032$). Last-token logit-lens accuracy rises from layer~22 to 41.6\% at
layer~26---upper layers partially transfer count information toward an
output-aligned subspace, but only to 42\%.

\paragraph{Subspace geometry.}
Overall mean $|\cos| = 0.016$ between probe directions and digit rows (bootstrap
95\% CI: $[0.015, 0.016]$), with $|\cos| \leq 0.032$ at every layer. A
...
A locus ablation confirms Q/V yields the best logit-lens alignment of any
...
repair generalizes: Mistral-7B achieves 92.0\% held-out; Pythia-410M achieves
31.4\% held-out. At 14B, $|\cos|$ sharpens to $0.011$ ($0.57\times$ random
...
synthetic prompts. MMLU shows no bottleneck (baseline 70.2\%; output-row
adaptation degrades to 55.6\%; $|\cos| = 0.31$--$0.48$ vs.\ $\leq 0.032$ for
counting). On DROP (single-digit subset), probe-round improves from 20.0\% to
...
Chain-of-thought (CoT) prompting achieves 89.0\% on entity counting (same
protocol, single-seed baseline), placing it alongside LoRA Q/V (83.1\%$\pm$7.2\%)
as an effective intervention. However, CoT and our approach address the
bottleneck through fundamentally different mechanisms. CoT externalizes the
...
$\sigma \in \{0.3, 0.5, 1.0\}$), and CoT comparison (CoT baseline: 89.0\%)
